...
Castle::Castle()
{
    ...
    for(int i = 0; i < 11; ++i)
    {
        ...
        if(i < 5)
        guards.__emplace_back(new IndenturedGuard(init));
        else if(i >= 5 && i < 9)
        guards.__emplace_back(new RovingGuard(init));
        else
        guards.__emplace_back(new HunterGuard(init, *hero));
    }
}
...
void Castle::carry()
{
    if(hero->pos->x == wife->pos->x && hero->pos->y == wife->pos->y)
        hasWife = true;
}
